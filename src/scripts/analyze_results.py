...
def analyze_by_model(results: list[dict]) -> dict:
    ...
    stats = defaultdict(
        lambda: defaultdict(
            lambda: {"accuracies": [], "variances": [], "variances_exclusive": [], "errors": [], "aborted": 0}
        )
    )
...
def print_approach_summary(stats: dict):
    ...
    for approach, data in sorted(stats.items()):
        ...
        if data["mean_variance"] is not None:
            var_ex_str = (
                f" (excl. failed: {data['mean_variance_exclusive']:.4f})"
                if data["mean_variance_exclusive"] is not None
                else ""
            )
...
def print_model_summary(stats: dict):
    ...
    for model, approaches in sorted(stats.items()):
        ...
        for approach, data in sorted(approaches.items()):
            ...
            print(
                f"  {approach:12} - Accuracy: {acc_str} | "
...
def main():
    ...
    parser.add_argument(
        "--csv", default=str(PROJECT_ROOT / "aggregated_results.csv"), help="Path to aggregated results CSV"
    )
    parser.add_argument("--export", help="Export summary to CSV file")
    parser.add_argument("--show-gains", action="store_true", help="Show NLT accuracy gains over structured approach")
    parser.add_argument("--show-perturbation", action="store_true", help="Show perturbation robustness analysis")
